...
@admin_sales_bp.route('/admin/sales', methods=['GET', 'POST'])
def admin_sales_section():
    '''
    ...
    if request.method == 'POST':
        ...
        if calc_type == 'amount':
            try:
                ...
                customer_id = request.form.get('customer_id','admin')
                record_sales(customer_id, total_amount=kg, price_per_kg=price_per_kg, kg_bought=amount)
            except ValueError:
                amount = None
            ...
            try:
                ...
                kg = None
    return render_template('admin_sales.html', kg=kg, amount=amount, current_price=price_per_kg)
